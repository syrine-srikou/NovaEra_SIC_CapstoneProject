...
def classify_weather_type(new_train, test):
    ...
    # Local directory where models will be saved and loaded from
    # Ensure the 'Fitted_Standardizers' folder exists
    if not os.path.exists('ClassifiedWeatherTypes'):
        os.makedirs('ClassifiedWeatherTypes')
    models_directory = 'ClassifiedWeatherTypes'  # Your local folder for models
    ...
    for interval in config['time_intervals']:
        ...
        try:
            ...
            model_filename = f'{models_directory}/RF_Weather_{interval}_.pkl'
            grid = joblib.load(model_filename)
            classified_weather_type = predict_weather_type(grid, interval_test_dataset[config['predictors']].copy())
        except FileNotFoundError:
            # If model not found, train a new one
            ...
            # Predict weather type using the newly trained model
            classified_weather_type = predict_weather_type(grid, interval_test_dataset[config['predictors']].copy())
...
def standardize_data(new_train, new_test):
    ...
    # Ensure the 'Fitted_Standardizers' folder exists
    if not os.path.exists('Fitted_Standardizers'):
        os.makedirs('Fitted_Standardizers')
    
    # Save fitted standardizer (scaler) locally in the 'Fitted_Standardizers' folder
    try:
    ...
    # Rebuild the DataFrames with the standardized data
    ...
    # Concatenate the standardized data with the other variables
...
def train_predict_MLP_model(new_stand_train, new_stand_test):
    ...
    # Ensure the 'Fitted_Models' folder exists
    if not os.path.exists('Fitted_Models'):
        os.makedirs('Fitted_Models')
    models_directory = 'Fitted_Models'  
    ...
    for interval, weather_type in product(config['time_intervals'], config['weather_types']):
        ...
        try:
            # Load the model from the local directory
            model_filename = f'{models_directory}/MLP_fitted_{interval}_{weather_type}.pkl'
            md = joblib.load(model_filename)
            predictions = md.predict(X_test)
        except:
            md = train_MLP_regressor(X_train, y_train)
            joblib.dump(md, f'{models_directory}/MLP_fitted_{interval}_{weather_type}.pkl') #save fitted model
            predictions = md.predict(X_test)
...
def train_predict_XGB_model(new_stand_train, new_stand_test):
    ...
    # Ensure the 'XGBFitted_Models' folder exists
    if not os.path.exists('Fitted_Models'):
        os.makedirs('Fitted_Models')
    models_directory = 'Fitted_Models'  
    ...
    for interval, weather_type in product(config['time_intervals'], config['weather_types']):
        ...
        try:
            model_filename = f'{models_directory}/XGB_fitted_{interval}_{weather_type}.pkl'
            md = joblib.load(model_filename)
            predictions = md.predict(X_test)
        except:
            md = train_XGB_regressor(X_train, y_train)
            joblib.dump(md, f'{models_directory}/XGB_fitted_{interval}_{weather_type}.pkl') #save fitted model
            predictions = md.predict(X_test)
...
def train_predict_RF_model(new_stand_train, new_stand_test):
    ...
    for interval, weather_type in product(config['time_intervals'], config['weather_types']):
        ...
        try:
            ...
            model_filename = f'{models_directory}/RF_fitted_{interval}_{weather_type}.pkl'
            md = joblib.load(model_filename)
            predictions = md.predict(X_test)
        except:
            md = train_RF_regressor(X_train, y_train)
            joblib.dump(md, f'{models_directory}/RF_fitted_{interval}_{weather_type}.pkl') #save fitted model
            predictions = md.predict(X_test)
...
def train_SHAP_values(df):
    """ This function calculates SHAP values for XGB and RF models by iterating through each interval and weather type. 
    The output is a DataFrame containing SHAP values, which can be converted into a NumPy array for use in summary plots."""
    ...
    for interval, weather_type in product(config['time_intervals'], config['weather_types']):
        ...
        XGB_gr = joblib.load(f'./Fitted_Models/XGB_fitted_{interval}_{weather_type}.pkl')
        XGB_estimator = XGB_gr.best_estimator_
        RF_gr = joblib.load(f'./Fitted_Models/RF_fitted_{interval}_{weather_type}.pkl')
        ...
        XGB_shap_values = XGB_explainer(subset_data)
    ...
    XGB_shap_df.to_csv("XGB_Shap.csv", index=False)
    RF_shap_df.to_csv("RF_Shap.csv", index=False)
    MLP_shap_df.to_csv("MLP_Shap.csv", index=False)

    return XGB_shap_df, RF_shap_df, MLP_shap_df

def import_SHAP_values():
    MLP_shap_df = pd.read_csv("./Fitted_Models/MLP_Shap.csv", sep = '\t')
    XGB_shap_df = pd.read_csv("./Fitted_Models/XGB_Shap.csv", sep = '\t')
    RF_shap_df = pd.read_csv("./Fitted_Models/RF_Shap.csv", sep = '\t')
...
def main():
    ...
    evaluate_model(forecasted_data_XGB, test, new_test_data)
